...
export default function Register() {
    ...
    function verify () {
        setState({...state, alert : {severity : "info", text : "Verifying..."}, disabled : true});
        axios.post('/api/verify', {email : state.email, code : Number(state.code)})
        .then(res => {
    ...
    function register () {
        setState({...state, alert : {severity : "info", text : "Registering..."}, disabled : true});
        if (!EmailValidator.validate(state.email)) {setState({...state, alert : {severity : "error", text: "Email invalid"}, disabled : false}); return;}
            ...
            <Box sx={{backgroundColor: 'primary.dark', display : 'flex', alignItems: 'center', justifyContent: 'center', minHeight : '100vh', height: '100%'}}>
                <Box sx={{minHeight : '50vh', width : isMobile ? '100vw' : '50vw'}}>
                    <div style={{margin : '50px'}}>
                        ...
                        {state.alert.text ? <Alert sx={{marginTop: '10px', marginBottom: '30px'}} severity={state.alert.severity}>{state.alert.text}</Alert> : <div/>}
                        <TextField sx={{marginBottom: '20px', input : {color : 'white'}}} color="secondary" disabled={state.waiting} variant="filled" type="text" onChange={(e) => {setState({...state, user : e.target.value})}} label="Username" required fullWidth /> 
                        <TextField sx={{marginBottom: '20px', input : {color : 'white'}}} color="secondary" disabled={state.waiting} variant="filled" type="email" onChange={(e) => {setState({...state, email : e.target.value})}} label="Email" required fullWidth/>
                        <TextField sx={{marginBottom: '20px', input : {color : 'white'}}} color="secondary" disabled={state.waiting} variant="filled" type="password" onChange={(e) => {setState({...state, pwd : e.target.value})}} label="Password" required fullWidth/>
                        ...
                        <div style={{display: 'flex', justifyContent: 'left', width: '100%'}}>
                          <HCaptcha theme='dark' sitekey="adbdce6c-dde2-46e1-b881-356447110fa7" onVerify={(token) => {setState({...state, htoken : token})}}/>
                        </div>
                        <div style={{display: 'flex', justifyContent: isMobile ? 'left' : 'end', alignItems: 'center', width: '100%', marginTop: isMobile ? '20px' : '0px'}}>
                          <Button disabled={state.disabled || (state.waiting ? !(state.code && isNumber(state.code) && state.code.length === 6) : !(state.htoken && state.user && state.email && state.pwd && state.sex))} type="submit" sx={{fontSize: '16px', height: '40px'}} color="secondary" variant="contained" onClick={state.waiting ? verify : register}>{state.waiting ? 'Verify' : 'Register'}</Button>
                        </div>
                        </div>
                    </div>
                </Box>
