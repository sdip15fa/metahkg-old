...
function ChooseCat (props:{errorHandler: Function, changeHandler: Function}) {
  const [state, setState] = React.useState<{data:any, cat: number}>({data : {}, cat : 0});
  useEffect(() => {
  ...
  return (
    <div>
      {!Object.keys(state.data).length ? <div/> : 
      <FormControl sx={{minWidth: '200px'}}>
      <InputLabel color="secondary">Category</InputLabel>
      <Select color="secondary" value={state.cat}
...
export default function Create(props:any) {
    ...
    function create () {
        setState({...state, alert : {severity : "info", text : "Creating topic..."}, disabled : true});
        axios.post('/api/create', {title : state.title, category : state.cat,
        icomment : state.icomment, htoken : state.htoken})
        .then(res => {
        ...
        return (
            <Box sx={{backgroundColor: 'primary.dark', minHeight: '100vh', display: 'flex', justifyContent: 'center'}}>
                <div style={{width: isMobile ? '100vw' : '80vw'}}>
                  ...
                  {state.alert.text ? <Alert sx={{marginTop: '10px', marginBottom: '10px'}} severity={state.alert.severity}>{state.alert.text}</Alert> : <div/>}
                  <TextField style={{marginBottom : '20px'}} variant="outlined" color="secondary" fullWidth label="Title" 
                  onChange={(e) => {setState({...state, title : e.target.value})}}/>
                  <TextEditor changehandler={(v:any,e:any) => {setState({...state, icomment : e.getContent()})}} text=''/>
                  <div style={{marginTop: '20px'}}>
                    <ChooseCat changeHandler={(e:any) => {setState({...state, cat : e.target.value})}} 
                    errorHandler={(e:any) => {setState({...state, alert : {severity : "error", text : e}})}}/>
                  </div>
                  <div style={isMobile ? {marginTop: '20px'} : {display: 'flex', flexDirection: 'row', width: '100%', marginTop: '20px'}}>
                    ...
                    <div style={{display: 'flex', justifyContent: isMobile ? 'left' : 'end', alignItems: 'center', width: '100%'}}>
                      <Button disabled={state.disabled || !(state.icomment && state.title && state.htoken && state.cat)} style={{marginTop: '20px', fontSize: '16px', height: '40px'}} onClick={create} variant='contained' color='secondary'>Create</Button>
                    </div>
                  </div>
                  </div>
                </div>
