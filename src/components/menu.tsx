...
function Menu (props:{id : string | number, category: string | number}) {
    ...
    async function fetch() {
        ...
        await axios.get(`/api/newest/${c}`)
        .then(res => {
          setData(res.data);})
        await axios.get(`/api/categories/${c}`)
        .then(res => {
    ...
    if (!data.length) {fetch()};
      return (
        <Box sx={{backgroundColor: 'primary.dark', maxWidth: '100%', minHeight: '100vh'}}>
          <Box sx={{backgroundColor: 'primary.main', width: '100%'}}>
            <div style={{display: 'flex', flexDirection: 'row', width: '100%', alignItems: 'center', height: '50px'}}>
              ...
              <div style={{display: 'flex', width: '100%', alignItems: 'center', justifyContent: 'center'}}>
                <p style={{textAlign: 'center', color : '#F5BD1F', fontSize: '18px', marginTop: '0px', marginBottom: '0px'}}>{cat}</p>
              </div>
          ...
          {!data.length ? <LinearProgress style={{width: '100%'}} color="secondary"/> :
