...
import { Mail as MailIcon, MoveToInbox as InboxIcon, Menu as MenuIcon } from '@mui/icons-material';
export default class SideBar extends React.Component<any> {
    ...
    render () {
        return (
            <div>
            <div>
              <Button sx={{margin: '10px', height: '40px', width:'40px'}} variant="text" onClick={this.toggleDrawer(true)}><MenuIcon style={{color : 'white'}}/></Button>
            </div>
        ...
        {['Sign in', 'Register', 'Create Topic'].map((text, index) => (
          <a href={this.links[1][index]}><ListItem button key={text}>
      ...
      <List>
        {['About', 'Source code'].map((text, index) => (
          <a href={this.links[2][index]}><ListItem button key={text}>
            <ListItemIcon>
            </ListItemIcon>
            <ListItemText primary={text} />
          </ListItem></a>))}
