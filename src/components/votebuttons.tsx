...
const [alert, setAlert] = useState<{error: boolean, open: boolean, text: string}>({error: false, open: false, text: ""})
const sendvote = (v:"up"|"down") => {
    v === "up" ? setUp(up + 1) : setDown(down + 1);
    setVote(v);
    axios.post('/api/vote', {id: Number(props.id), cid: Number(props.cid), vote: v})
    .catch((err) => {
