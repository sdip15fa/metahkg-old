...
router.post('/api/vote', body_parser.json(), async (req, res) => {
    if (!req.body.id || !req.body.cid || !req.body.vote ||
        Object.keys(req.body).length > 3 || !(typeof req.body.cid === "number" &&
        typeof req.body.id === "number" && ["up","down"].includes(req.body.vote))) {
            res.status(400);
    ...
    try {
        ...
        if (!thread || !thread.conversation[req.body.cid]) {res.status(404); res.send("Not found."); return;}
